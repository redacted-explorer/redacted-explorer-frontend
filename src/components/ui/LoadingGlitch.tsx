...
const GlitchText = ({ children, isLoading }) => {
  ...
  const createGlitchRect = useCallback((color) => {
    ...
    rect.setAttribute('opacity', Math.random() * 0.7 + 0.3);
    return rect;
  }, []);

  const animateGlitch = useCallback((color) => {
  ...
  useEffect(() => {
    let glitchInterval1, glitchInterval2, glitchInterval3, highlightInterval;
